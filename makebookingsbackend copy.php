    ...
    if (isset($_POST['submit'])) {
        ...
        if (!isset($_SESSION['is_admin'])) {
            ...
            foreach ($booked_in_slots as $booked_in_slot) {
                if ($booked_in_slot['day'] == $day) {
                    if (trim($booked_in_slot['start_time']) == trim($start_timing) || trim($booked_in_slot['end_time']) == trim($end_timing)) {
                        $error = 1;
                        array_push($error_messages, "Please try a different lesson length. ");
                    }
                }
            }
            ...
            # finding user_id
            $sql = "SELECT user_id FROM users WHERE paul_usersemail = '$email'";
            $result = mysqli_query($conn, $sql);
            ...
            if (count($error_messages) > 0) {
                ...
                $sql = "INSERT INTO booked_in_slots (paul_usersid, start_time, end_time, day, instrument) VALUES ('$paul_usersid', '$start_timing', '$end_timing', '$day', '$instrument')";
                $result = mysqli_query($conn, $sql);
                echo "Lesson added!";
