    ...
    <?php
        ...
        if(isset($_POST['submit'])){
            ...
            if($_SESSION['attempts'] >= 3){
                ...
                $sql = "SELECT paul_userspassword FROM users WHERE paul_usersemail = '$email'";
                $result = mysqli_query($conn, $sql);
                $info = mysqli_fetch_assoc($result);
                $password_hashed = $info['paul_userspassword'];
                ...
                $sql = "SELECT * FROM users WHERE paul_usersemail = '$email'";
                $result = mysqli_query($conn, $sql);
                $info = mysqli_fetch_assoc($result);
